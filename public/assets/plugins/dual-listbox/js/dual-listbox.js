...
;(function ($) {
    ...
    var Transfer = function (element, options) {
        ...
        //exclude
        this.exclude = this.settings.exclude;
        //include
        this.include = this.settings.include;
        //input name
        this.inputName = this.settings.inputName;
        // inner data
        this._data = new InnerMap();
        // inner group data
        this._group_data = new InnerMap();

        this.selectedItems = this.settings.selectedItems;
        // Id
        this.id = (getId())();
    ...
    Transfer.prototype.generate_transfer = function () {
        ...
        var template = parseHTMLTemplate(function () {
            /*
            <div class="card" id="transfer_double_{{= self.id }}">
                <div class="transfer-double-content row">
                    ...
                    <div class="col-md-2 align-self-center text-center">
                        <div class="btn btn-secondary" id="add_selected_{{= self.id }}">{{= self.include }}</div>
                        <div class="btn btn-secondary" id="delete_selected_{{= self.id }}">{{= self.exclude }}</i></div>
                    </div>
                        ...
                        <div class="transfer-double-selected-list">
                            <div class="transfer-double-selected-list-header">
                                <div class="transfer-double-selected-list-search">
                                    <input class="form-control" type="text" id="selectedListSearch_{{= self.id }}" placeholder="{{= self.searchPlaceholderText }}" value="" />
                                </div>
                            </div>
            ...
            <div class="transfer-double-list transfer-double-list-{{= self.id }}">
                <div class="transfer-double-list-header">
                    <div class="transfer-double-list-search">
                        <input class="form-control" type="text" id="groupListSearch_{{= self.id }}" placeholder="{{= self.searchPlaceholderText }}" value="" />
                    </div>
                </div>
    ...
    Transfer.prototype.generate_items_container = function () {
        ...
        var template = parseHTMLTemplate(function () {
            ...
            <div class="transfer-double-list transfer-double-list-{{= self.id }}">
                <div class="transfer-double-list-header">
                    <div class="transfer-double-list-search">
                        <input class="form-control" type="text" id="listSearch_{{= self.id }}" placeholder="{{= self.searchPlaceholderText }}" value="" />
                    </div>
                </div>
                <div class="transfer-double-list-content">
                    <div class="transfer-double-list-main">
        ...
        for (var i = 0; i < groupDataArray.length; i++) {
            if (groupDataArray[i][groupArrayName] && groupDataArray[i][groupArrayName].length > 0) {
                ...
                for (var j = 0; j < groupDataArray[i][groupArrayName].length; j++) {
                    var selected = groupDataArray[i][groupArrayName][j].selected || false || ($.inArray(groupDataArray[i][groupArrayName][j][valueName].toString(), this.selectedItems) >= 0);
                    var right_total_count = this._data.get("right_total_count") || 0;
                    this._data.get("right_total_count") == undefined ? this._data.put("right_total_count", right_total_count) : void (0)
                    selected ? this._data.put("right_total_count", ++right_total_count) : void (0)
    ...
    Transfer.prototype.generate_right_group_items = function () {
        ...
        for (var i = 0; i < groupDataArray.length; i++) {
            if (groupDataArray[i][groupArrayName] && groupDataArray[i][groupArrayName].length > 0) {
                for (var j = 0; j < groupDataArray[i][groupArrayName].length; j++) {
                    if (groupDataArray[i][groupArrayName][j].selected || false || ($.inArray( groupDataArray[i][groupArrayName][j][valueName].toString(), this.selectedItems) >= 0)) {
                        this._data.put("right_total_count", ++selected_count);
