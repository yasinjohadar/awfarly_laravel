 ...
 *
 * Pure, so every shape Evolution has shipped can be pinned by a unit test. "I could not
 * read it" is null, and the callers must decide what to do with null — never guess "close".
 */
final class EvolutionInstanceState
{
     ...
     * the admin form and may contain spaces, so a stray trailing space must not read as
     * "this phone is disconnected". Returns null when the name is genuinely absent —
     * deliberately no "fall back to the first row", which would copy a different phone's
     * JID and number onto this one.
     *
     ...
     * "number" would leave the admin table showing "—" for a connected phone.
     */
