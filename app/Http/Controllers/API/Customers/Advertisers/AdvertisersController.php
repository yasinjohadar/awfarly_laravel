...
class AdvertisersController extends Controller
{
    ...
    public function getAdvertiserById($id)
    {
        ...
        if (!$advertiser) {
            return $this->apiBadRequestResponse(__('api/customers/advertisers/advertisers.wrong-id'));
        }
        ...
        //get followed advertisers
        $followed_advertiser = Auth::guard('customer-api')->user()
            ->followed()
            ->where('followed_type', AdvertiserUser::class)
        ...
        if ($advertiser->status === 'inactive') {
            return $this->apiBadRequestResponse(__('api/customers/advertisers/advertisers.status-closed'));
        } elseif ($advertiser->status === 'banned') //return error if account is banned
        {
            return $this->apiBadRequestResponse(__('api/customers/advertisers/advertisers.status-banned'));
        }
    ...
    public function rateAdvertisers($id, Request $request)
    {
        ...
        if ($already_rated) {
            return $this->apiBadRequestResponse(__('api/customers/advertisers/advertisers.rating.already-rated'));
        }
        DB::beginTransaction();
        try {
            ...
            DB::rollBack();
            return $this->apiExceptionResponse(__('api/customers/advertisers/advertisers.rating.something-wrong'));
        }
        DB::commit();
        ...
        return $this->apiResponse([
            'message' => __('api/customers/advertisers/advertisers.rating.rated-successfully'),
            'data' => [
    ...
    public function usersRated(Request $request)
    {
        ...
        $rater_users = Auth::guard('customer-api')->user()
            ->advertisersRated()
            ->join('advertisers_users', function ($query) use ($advertisers_blocks) {
                return $query->on('advertisers_users.id', 'advertisers_ratings.advertiser_id')
                    ->where('advertisers_users.status', 'active')
                    ->whereNotIn('advertisers_users.id', $advertisers_blocks);
            })
            ->where('advertisers_ratings.status', 'approved')
            ->paginate($limit);
