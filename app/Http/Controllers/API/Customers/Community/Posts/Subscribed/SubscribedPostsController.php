...
namespace App\Http\Controllers\API\Customers\Community\Posts\Subscribed;

use App\Helpers\Settings;
use App\Http\Controllers\Controller;
use App\Http\Resources\Customers\Community\Posts\CommunityPostsResource;
use App\Http\Resources\Customers\Community\Posts\Subscribed\SubscribedPostsResource;
        ...
        try {
            ...
            if ($request->has('isSubscribed')) {
                if ($request->get('isSubscribed')) {
                    ...
                    $isSubscribed = true;
                    $type = __('api/customers/community/posts/subscribed-posts.subscribed');
                } else {
                    if ($hidden_post) {
                        $hidden_post->delete();
                    }
                    $isSubscribed = false;
                    $type = __('api/customers/community/posts/subscribed-posts.unsubscribed');
                }
            } else {
                if ($hidden_post) {
                    $hidden_post->delete();
            ...
            DB::rollBack();
            return $this->apiExceptionResponse(__('api/customers/community/posts/subscribed-posts.something-wrong'));
        }
        DB::commit();
        return $this->apiResponse([
            'message' => __('api/customers/community/posts/subscribed-posts.follow-toggle', ['toggle' => $type]),
            'data' => [
        ...
        $posts = Post::query()
            ...
            ->join('posts_subscriptions', function ($q) {
                return $q->on('posts_subscriptions.post_id', 'posts.id')
                    ->where('posts_subscriptions.user_type', CustomerUser::class)
                    ->where('posts_subscriptions.user_id', Auth::guard('customer-api')->id());
            })
