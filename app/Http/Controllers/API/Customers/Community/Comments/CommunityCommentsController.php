...
namespace App\Http\Controllers\API\Customers\Community\Comments;
...
use App\Http\Resources\Customers\Community\Comments\Reports\ReportedCommentsResource;
...
class CommunityCommentsController extends Controller
{
    ...
    public function addPostComment($id, Request $request)
    {
        ...
        if (!$post) {
            return $this->apiBadRequestResponse(__('api/customers/community/comments/comments.wrong-post'));
        }
        ...
        try {
            //add the comment
            $comment = Auth::guard('customer-api')->user()
                ->postsComments()
                ->create($data);
            ...
            //roll back
            DB::rollBack();
            return $this->apiExceptionResponse(__('api/customers/community/comments/comments.something-wrong'));
        }
        //commit
        DB::commit();
        ...
        $isPostOwner = ($post->user && $post->user->user_type === 'customer' && $post->user->id == Auth::guard('customer-api')->id());
        ...
        if ($data['comment_id']) {
            ...
            if ($parentCommentAuthor && !$isReplyToSelf) {
                Notifications::notifyCommentReply($parentCommentAuthor, 'posts.comments', Auth::guard('customer-api')->user()->name, $customProperties);
            }
        }

        return $this->apiResponse([
            'message' => __('api/customers/community/comments/comments.comment-added'),
    ...
    public function editPostComment($id, Request $request)
    {
        ...
        //return error if post wasn't found
        if (!$comment) {
            return $this->apiBadRequestResponse(__('api/customers/community/comments/comments.wrong-id'));
        }
        ...
        DB::commit();
        return $this->apiResponse([
            'message' => __('api/customers/community/comments/comments.comment-edited'),
            'data' => CommunityCommentsResource::make($comment),
        ]);
    }
    ...
    public function deletePostComment($id)
    {
        ...
        DB::commit();
        return $this->apiResponse([
            'message' => __('api/customers/community/comments/comments.comment-deleted'),
        ]);
    }
    ...
    public function reportComment($comment_id, Request $request)
    {
        ...
        //return error if comment user is banned
        ...
        DB::commit();
        return $this->apiResponse([
            'message' => __('api/customers/community/comments/comments.reports.report-added'),
    ...
    public function editCommentReport(Request $request, $id)
    {
        ...
        //return error if post wasn't found
        if (!$report) {
            return $this->apiBadRequestResponse(__('api/customers/community/comments/comments.reports.no-report'));
        }
        ...
        DB::commit();
        return $this->apiResponse([
            'message' => __('api/customers/community/comments/comments.reports.report-edited'),
            'data' => ReportedCommentsResource::make($report),
        ]);
    }
    ...
    public function toggleLikeComment($id, Request $request)
    {
        ...
        try {
            ...
            if ($request->has('isLiked')) {
                if ($request->get('isLiked')) {
                    ...
                    $isLiked = true;
                    $type = __('api/customers/community/comments/comments.liked');
                } else {
                    if ($like) {
                        $like->delete();
                        $comment->likes_count -= 1;
                    ...
                    $isLiked = false;
                    $type = __('api/customers/community/comments/comments.disliked');
                }
            } else {
                if ($like) {
                    $like->delete();
        ...
        if ($isLiked && !($comment->user->user_type === 'customer' && $comment->user_id == Auth::guard('customer-api')->id())) {
            $customProperties = [
        ...
        return $this->apiResponse([
            'message' => __('api/customers/community/comments/comments.like-toggle', ['toggle' => $type]),
            'data' => [
                'isLiked' => $isLiked,
            ]
        ]);
