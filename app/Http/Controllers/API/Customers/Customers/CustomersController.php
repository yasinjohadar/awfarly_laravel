...
namespace App\Http\Controllers\API\Customers\Customers;
...
class CustomersController extends Controller
{
    ...
    public function getCustomerById($id)
    {
        ...
        if (!$customer) {
            return $this->apiBadRequestResponse(__('api/customers/customers/customers.wrong-id'));
        }
        ...
        if ($block || $blocked) {
            return $this->apiBadRequestResponse(__('api/customers/customers/customers.unavailable'));
        }
        //return error if account is closed
        if ($customer->status === 'inactive') {
            return $this->apiBadRequestResponse(__('api/customers/customers/customers.status-closed'));
        } elseif ($customer->status === 'banned') //return error if account is banned
        {
            return $this->apiBadRequestResponse(__('api/customers/customers/customers.status-banned'));
        }
