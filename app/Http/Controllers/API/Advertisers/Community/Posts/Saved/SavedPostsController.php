...
namespace App\Http\Controllers\API\Advertisers\Community\Posts\Saved;

use App\Helpers\Settings;
use App\Http\Controllers\Controller;
use App\Http\Resources\Advertisers\Community\Posts\CommunityPostsResource;
use App\Http\Resources\Advertisers\Community\Posts\Saved\SavedPostsResource;
        ...
        try {
            ...
            if ($request->has('isSaved')) {
                if ($request->get('isSaved')) {
                    ...
                    $isSaved = true;
                    $type = __('api/advertisers/community/posts/saved-posts.saved');
                } else {
                    if ($saved_post) {
                        $saved_post->delete();
                    }
                    $isSaved = false;
                    $type = __('api/advertisers/community/posts/saved-posts.unsaved');
                }
            } else {
                if ($saved_post) {
                    $saved_post->delete();
            ...
            DB::rollBack();
            return $this->apiExceptionResponse(__('api/advertisers/community/posts/saved-posts.something-wrong'));
        }
        DB::commit();
        return $this->apiResponse([
            'message' => __('api/advertisers/community/posts/saved-posts.follow-toggle', ['toggle' => $type]),
            'data' => [
                'isSaved' => $isSaved,
            ],
        ]);
        ...
        $posts = Post::query()
            ...
            ->join('saved_posts', function ($q) {
                return $q->on('saved_posts.post_id', 'posts.id')
                    ->where('saved_posts.user_type', AdvertiserUser::class)
                    ->where('saved_posts.user_id', Auth::guard('advertiser-api')->id());
            })
