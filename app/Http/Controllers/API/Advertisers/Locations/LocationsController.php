...
namespace App\Http\Controllers\API\Advertisers\Locations;
...
class LocationsController extends Controller
{
    ...
    public function addUserLocations(Request $request)
    {
        ...
        if (!empty($cityIds)) {
            ...
            if ($cities->count() !== count($cityIds)) {
                return $this->apiBadRequestResponse(__('api/advertisers/locations/locations.invalid-city'));
            }
        ...
        try {
            ...
            DB::rollBack();
            return $this->apiExceptionResponse(__('api/advertisers/locations/locations.something-wrong'));
        }

        DB::commit();

        return $this->apiResponse([
            'message' => __('api/advertisers/locations/locations.added'),
            'data' => [
                'governorates' => GovernoratesResource::collection($governorates),
                'cities' => CitiesResource::collection($cities),
            ],
    ...
    public function deleteUserLocations(Request $request)
    {
        ...
        return $this->apiResponse([
            'message' => __('api/advertisers/locations/locations.deleted'),
            'data' => [
                'governorates' => GovernoratesResource::collection($governorates),
                'cities' => CitiesResource::collection($cities),
            ],
