...
namespace App\Http\Controllers\API\Advertisers\Chat;
...
use App\Http\Resources\Advertisers\Chats\ChatMessagesResource;
use App\Http\Resources\Advertisers\Chats\ChatsChannelsResource;
use App\Http\Resources\Advertisers\Chats\ChatsUsersResource;
...
class ChatController extends Controller
{
    ...
    {
        //get chats
        $chats = Auth::guard('advertiser-api')->user()
            ->chats();
        ...
        if ($token) {
            ...
            if (!$chat) {
                return $this->apiBadRequestResponse(__('api/advertisers/chat/chats.wrong-token'));
            }
    ...
    public function createChat(Request $request)
    {
        ...
        //get user chats
        $user_chats = Auth::guard('advertiser-api')->user()
            ->chatsUsers()
            ->pluck('chat_id');

        //get user data
        if ($data['toUserType'] === 'advertiser') {
            if ($data['toUserId'] == Auth::guard('advertiser-api')->id()) {
                return $this->apiBadRequestResponse(__('api/advertisers/chat/chats.self-chat'));
            }
            //get user
        ...
        if (!$user) {
            return $this->apiBadRequestResponse(__('api/advertisers/chat/chats.wrong-user'));
        }
        ...
        if (!$chat) {
            ...
            if ($user->chats_privacy === 'followers') {
                ...
                $user_followed = Auth::guard('advertiser-api')->user()
                    ...
                    ->first();
                if (!$user_followed) {
                    return $this->apiBadRequestResponse(__('api/advertisers/chat/chats.chat-private'));
                }
            } else if ($user->chats_privacy === 'disabled') {
                return $this->apiBadRequestResponse(__('api/advertisers/chat/chats.chat-disabled'));
            }
        }
        if (!$chat) {
            ...
            if ($block || $blocked) {
                $message = $user->user_type === 'advertiser' ? __('api/advertisers/advertisers/advertisers.unavailable') : __('api/advertisers/customers/customers.unavailable');
        ...
        try {
            ...
            DB::rollBack();
            return $this->apiExceptionResponse(__('api/advertisers/chat/chats.something-wrong'));
        }
        DB::commit();
        try {
            return $this->apiResponse([
                'message' => __('api/advertisers/chat/chats.chat-created'),
    ...
    public function sendMessage(Request $request, $token)
    {
        ...
        $other_user = $chat->users()
            ->where(function ($q) {
                return $q->where(function ($q) {
                    return $q->where('user_id', '!=', Auth::guard('advertiser-api')->id());
                })
                    ->orWhere(function ($q) {
                        return $q->where('user_type', '!=', AdvertiserUser::class);
                    });
            })
            ->first()
            ->user;
        ...
        if ($block || $blocked) {
            $message = $other_user->user_type === 'advertiser' ? __('api/advertisers/advertisers/advertisers.unavailable') : __('api/advertisers/customers/customers.unavailable');
            return $this->apiBadRequestResponse($message);
        }
        ...
        try {
            ...
            if ($request->hasFile('media')) {
                ...
                if (strstr($mime_type, "video/")) {
                    $message = __('api/advertisers/chat/chats.fcm.video');
                    $type = 'video';
                } else if (strstr($mime_type, "image/")) {
                    $message = __('api/advertisers/chat/chats.fcm.image');
                    $type = 'image';
                } else if (strstr($mime_type, "audio/")) {
                    $message = __('api/advertisers/chat/chats.fcm.audio');
                    $type = 'audio';
                } else {
                    $message = __('api/advertisers/chat/chats.fcm.file');
                    $type = 'file';
                }
            } else {
        ...
        try {
            //return with message
            return $this->apiResponse([
                'message' => __('api/advertisers/chat/chats.message-sent'),
            ...
            if ($other_user->fcm_token && !$other_user->is_online) {
                $senderName = Str::limit(Auth::guard('advertiser-api')->user()->name, 20);
                FcmHelper::sendFcmNotification([
                    'title' => trans('api/advertisers/chat/chats.fcm.title', ['name' => $senderName], 'ar'),
                    'title_en' => trans('api/advertisers/chat/chats.fcm.title', ['name' => $senderName], 'en'),
                    'body' => $message,
                    'body_en' => $message,
    ...
    public function deleteMessage($id)
    {
        ...
        $message = Auth::guard('advertiser-api')->user()
            ->messages()
            ->where('id', $id)
            ->first();

        if (!$message) {
            return $this->apiBadRequestResponse(__('api/advertisers/chat/chats.wrong-message-id'));
        }
        DB::beginTransaction();
        try {
            //set data
        ...
        DB::commit();
        return $this->apiResponse([
            'message' => __('api/advertisers/chat/chats.message-deleted'),
        ]);
    }
    ...
    {
        //get chats
        $chat = Auth::guard('advertiser-api')->user()
            ->chats()
            ->where('token', $token)
            ->first();
