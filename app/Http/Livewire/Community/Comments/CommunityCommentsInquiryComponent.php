...
class CommunityCommentsInquiryComponent extends LivewireDatatable
{
    use LivewireAlert;
    ...
    public $model = PostComments::class;
    ...
    public function columns(): array
    {
        return [
            ...
            NumberColumn::name('id')
                ...
                ->searchable(),
            Column::name('post_id')
                ->label(__('pages/community/comments/inquiry.datatable.post_id'))
                ->filterable()
                ->searchable()
                ->linkTo('admin/community/posts'),
            Column::callback(['id', 'created_at'], function ($id) {
                $user_type = optional(PostComments::withTrashed()
                    ->findOrFail($id)
                    ->user)->user_type;
                return ucwords($user_type);
            })
                ->label(__('pages/community/comments/inquiry.datatable.user_type'))
                ->unsortable(),
            Column::name('user_id')
                ->label(__('pages/community/comments/inquiry.datatable.user_id'))
                ->filterable()
                ->searchable(),
            Column::callback(['id'], function ($id) {
                $username = optional(PostComments::withTrashed()
                            ->findOrFail($id)
                            ->user)->name;
                return $username;
            })
                ->label(__('pages/community/comments/inquiry.datatable.user_name')),
            Column::callback(['comment'], function ($comment) {
                return Str::limit($comment, 30);
            })
                ->label(__('pages/community/comments/inquiry.datatable.comment'))
                ->filterable()
                ->searchable(),
            ...
            Column::callback(['id', 'updated_at', 'deleted_at'], function ($id, $name, $deleted_at) {
                return $deleted_at ? view('admin.pages.community.comments.table-actions', ['id' => $id, 'name' => $name, 'deleted_at' => $deleted_at]) : '-';
            })
    ...
    public function builder(): Builder
    {
        if ($this->page_type === 'deleted') {
            return PostComments::onlyTrashed()
                ->whereHas('post', function ($q) {
                    $q->whereNull('advertisement_id');
                });
        }/* else if ($this->page_type === 'active') {
            return PostComments::where('comments_count', '>', 0);
        }*/
        return PostComments::withTrashed()
            ->whereHas('post', function ($q) {
                $q->whereNull('advertisement_id');
            });
    }
    ...
    {
        //get user with data
        $this->comment = PostComments::withTrashed()
            ->where('id', $id)
            ->first()
            ->toArray();
    ...
    public function setModalTexts()
    {
        $this->deleteModalTexts = [
            'title' => __('pages/community/comments/inquiry.modal.delete.title'),
            'select-option' => __('pages/community/comments/inquiry.modal.delete.select-option'),
            'soft-delete' => __('pages/community/comments/inquiry.modal.delete.soft-delete'),
            'permanent-delete' => __('pages/community/comments/inquiry.modal.delete.permanent-delete'),
            'content' => __('pages/community/comments/inquiry.modal.delete.content'),
            'cancel' => __('pages/community/comments/inquiry.modal.delete.cancel'),
            'submit' => __('pages/community/comments/inquiry.modal.delete.submit'),
        ];
        $this->editModalTexts = [
            'title' => __('pages/community/comments/inquiry.modal.edit.title'),
            'cancel' => __('pages/community/comments/inquiry.modal.edit.cancel'),
            'submit' => __('pages/community/comments/inquiry.modal.edit.submit'),
        ];
        $this->restoreModalTexts = [
            'title' => __('pages/community/comments/inquiry.modal.restore.title'),
            'content' => __('pages/community/comments/inquiry.modal.restore.content'),
            'cancel' => __('pages/community/comments/inquiry.modal.restore.cancel'),
            'submit' => __('pages/community/comments/inquiry.modal.restore.submit'),
        ];
    }
    ...
    public function restore($id)
    {
        ...
        try {
            ...
            //restore comment
            $comment = PostComments::withTrashed()->find($id)->restore();
