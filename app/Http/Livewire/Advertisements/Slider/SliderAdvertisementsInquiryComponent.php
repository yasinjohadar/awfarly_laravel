...
namespace App\Http\Livewire\Advertisements\Slider;
...
class SliderAdvertisementsInquiryComponent extends LivewireDatatable
{
    use LivewireAlert;
    ...
    public $model = SliderAdvertisement::class;
    public array $user;
    ...
    public function columns(): array
    {
        return [
            ...
            Column::callback('advertisement_url', function ($advertiser_url) {
                ...
                ->label(__('pages/advertisements/slider/inquiry.content.datatable.url'))
                ->filterable()
                ->searchable()
                ->truncate(60)
                ->alignCenter(),
            DateColumn::callback('starts_at', function ($starts_at) {
                ...
                ->label(__('pages/advertisements/slider/inquiry.content.datatable.starts_at'))
                ->filterable()
                ->searchable()
                ->alignCenter(),
            DateColumn::callback('ends_at', function ($ends_at) {
                ...
                ->label(__('pages/advertisements/slider/inquiry.content.datatable.ends_at'))
                ->filterable()
                ->searchable()
                ->alignCenter()
                ->hide(),
            Column::callback(['id'], function ($id) {
                return view('admin.pages.advertisements.slider.table-actions', ['id' => $id]);
            })
    ...
    public function builder()
    {
        if ($this->page_type === 'active') {
            return SliderAdvertisement::where('ends_at', '>', now())
                ->orWhereNull('ends_at');
        } elseif ($this->page_type === 'expired') {
            return SliderAdvertisement::where('ends_at', '<', now());
        }
        return SliderAdvertisement::selectRaw('*');
    }
    ...
    public function deleteSelected()
    {
        ...
        try {
            ...
            //delete selected data
            $advertisements = SliderAdvertisement::whereIn('id', $this->selected)
                ->get();
    ...
    public function setModalTexts()
    {
        $this->deleteModalTexts = [
            'title' => __('pages/advertisements/slider/inquiry.modal.delete.title'),
            'content' => __('pages/advertisements/slider/inquiry.modal.delete.content'),
            'cancel' => __('pages/advertisements/slider/inquiry.modal.delete.cancel'),
            'submit' => __('pages/advertisements/slider/inquiry.modal.delete.submit'),
        ];
    }
}
