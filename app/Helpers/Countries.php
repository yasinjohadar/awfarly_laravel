...
use App\Models\Countries\Timezones\Timezone;
use App\Models\Payments\Currencies\PaymentsCurrency;
...
class Countries extends Helper
{
    ...
    */
    public static function GetNameFromCode($code)
    {
        ...
        if ($country)
            return $country->{$name_column};

        return '';
    }

    /*
    * Get currency from country code or get default currency
    */
    public static function GetCurrencyFromCode($code)
    {
        ...
        //Get currency
        $currency = PaymentsCurrency::where('country_code', $code)
            ->first();
        ...
        //return default
        return PaymentsCurrency::where('is_default', true)->first();
    }

    /*
    * Get all countries codes
    */
    public static function GetAllCountriesCodes()
    {
        return Country::select('code')
            ->orderBy('id', 'asc')
            ->pluck('code')
            ->toArray();
    }
   ...
   */
    public static function GetAllCountriesCodesAndNames()
    {
        $countries = [];

        $data = Country::select(['name', 'code'])
            ->orderBy('id', 'asc')
            ->get();
   ...
   */
    ...
    {
        return Timezone::pluck('timezone')
            ->toArray();
    }
  ...
  */
    public static function GetTZByCountryCode($code)
    {
        //Set code
        $code = Str::upper($code);

        $timezone = Timezone::where('country_code', $code)
            ->first();
   ...
   */
    ...
    {
        return Country::where('is_default', true)
            ->first();
    }
   ...
   */
    public static function GetGeoIPData()
    {
        ...
        return (object)[
            ...
            'country_code' => $geoip['countryCode'] ?? $default_country->code,
            'city' => $geoip['city'],
            'region' => $geoip['region'],
            'referrer' => $referer,
    ...
    /*
     * Get current user or default system country, timezone, currency
     */
    public static function GetCurrentUserOrDefaultCountryTimezoneCurrency($get_geo = true)
    {
        //Check authentication
        $is_authenticated = Auth::guard('customer-api')->check() || Auth::guard('advertiser-api')->check() || Auth::guard('admin')->check();

        //Get GeoIp
        $geoip = ($get_geo ? self::GetGeoIPData() : []);
        ...
        if ($is_authenticated) {
            ...
            } elseif (Auth::guard('advertiser-api')->check()) {
            ...
            //Set country code
            $county_code = $user->country_code;

            //Set timezone
            $timezone = $user->timezone;
        } else {
            //Set country code
            $county_code = ($get_geo ? $geoip->country_code : 'EG');
        }
        ...
        $country = Country::select([
            ...
            ->where('code', $county_code)
            ->first();

        //Get timezone from user default or from county
        $timezone = (isset($timezone) && !!$timezone ? $timezone : $country->timezone->timezone);
        ...
        //Get currency (If no currency for this country in the system return the default country currency)
        $currency = $country_currency ?? $default_country->currency;

        //Get currency country code (If no currency for this country in the system return the default country code)
        $currency_country_code = $country_currency ? $country->code : $default_country->code;
        ...
        return [
            ...
            'timezone' => $timezone,
            'currency' => $currency,
            'currency_country_code' => $currency_country_code,
  ...
  */
    public static function GetCountryCoordsByCodes($code)
    {
        $countries = [
            ...
            "SS" => ["name" => "S. Sudan", "coords" => [4.859363, 31.571251]],
            ...
            "TL" => ["name" => "Timor-Leste", "coords" => [-8.236871, 125.576729]],
