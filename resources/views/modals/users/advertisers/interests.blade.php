<x-dialog-modal wire:model="showInterestsModal">
    <x-slot name="title">
        {{ __('pages/advertisers/index.modal.interests.title', ['name' => $viewed_user_name]) }}
    </x-slot>
    <x-slot name="content">
        ...
        @else
            <div class="text-muted">
                {{ __('pages/advertisers/index.modal.interests.empty') }}
            </div>
        @endif
    </x-slot>
    <x-slot name="footer">
        <x-secondary-button wire:click="closeInterestsModal" wire:loading.attr="disabled">
            {{ __('pages/advertisers/index.modal.interests.close') }}
        </x-secondary-button>
    </x-slot>
</x-dialog-modal>
