<x-dialog-modal wire:model="showCategoriesModal">
    <x-slot name="title">
        {{ __('pages/customers/index.modal.categories.title', ['name' => $viewed_user_name]) }}
    </x-slot>
    <x-slot name="content">
        ...
        @else
            <div class="text-muted">
                {{ __('pages/customers/index.modal.categories.empty') }}
            </div>
        @endif
    </x-slot>
    <x-slot name="footer">
        <x-secondary-button wire:click="closeCategoriesModal" wire:loading.attr="disabled">
            {{ __('pages/customers/index.modal.categories.close') }}
        </x-secondary-button>
    </x-slot>
</x-dialog-modal>
