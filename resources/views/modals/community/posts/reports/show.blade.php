...
<x-dialog-modal wire:model="showMoreModal">
    ...
    <x-slot name="content">
        ...
        <h4>{{__('pages/community/posts/reports/show.content.status')}}
            <code>{{($log['status'] ?? null) === 'solved' ? __('pages/community/posts/reports/show.content.solved') : __('pages/community/posts/reports/show.content.unsolved')}}</code>
        </h4>
        @if(($log['status'] ?? null) === 'solved' && !empty($log['resolution']))
            <h4>{{__('pages/community/posts/reports/show.content.resolution')}}
                <code>{{$log['resolution']}}</code>
            </h4>
            @if(!empty($log['resolved_by']))
                <h4>{{__('pages/community/posts/reports/show.content.resolved_by')}}
                    <code>{{$log['resolved_by']}}</code>
                </h4>
            @endif
            @if(!empty($log['resolved_at']))
                <h4>{{__('pages/community/posts/reports/show.content.resolved_at')}}
