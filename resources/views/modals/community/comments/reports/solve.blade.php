<x-confirmation-modal wire:model="showSolveModal" type="restore">
    ...
    <x-slot name="content">
        ...
        <div class="form-group mb-0">
            <label>{{ __('pages/community/comments/reports/show.modal.solve.resolution_label') }}</label>
            <textarea class="form-control @error('resolution') is-invalid @enderror"
                      rows="3"
                      wire:model.defer="resolution"
                      placeholder="{{ __('pages/community/comments/reports/show.modal.solve.resolution_placeholder') }}"></textarea>
            @error('resolution')
