@props([
    ...
    'multiple' => false,
    'mode' => 'attachment',
    'profileClass' => 'w-100 h-100 rounded-full'
])
...
<div x-data="{
    ...
    @if ($mode === 'attachment')
        ...
        <div wire:loading.delay wire:loading.flex wire:target="removeUpload" wire:loading.class="w-100">
            <div class="text-danger bg-danger-100 flex-1 p-1 text-center rounded">
                Removing file...
            </div>
        </div>
        ...
        <div>
            @if(is_array($file) && count($file) > 0)
                @foreach($file as $key => $f)
                    <div class="py-3 flex {{ !$loop->last ? 'border-bottom-2 border-secondary' : '' }}">
                        <div class="w-50 mr-4 flex-shrink-0 shadow-sm rounded-lg" >
                            @if(collect(['jpg', 'png', 'jpeg', 'webp'])->contains($f->getClientOriginalExtension()))
                                <div class="position-relative pb-4 overflow-hidden rounded-lg border border-secondary-100">
                                    <img src="{{ $f->temporaryUrl() }}" class="w-100 h-100 position-absolute img-fluid rounded-lg">
                                </div>
                            @else
                                <div class="w-50 h-50 bg-secondary-100 text-primary flex align-items-center justify-content-center rounded-lg border border-secondary-100">
                                    <svg class="h-50 w-50" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
