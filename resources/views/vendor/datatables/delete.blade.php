<div x-data="{ working: false }" x-cloak wire:key="{{ $value }}">
    <span>
        <button type="button" class="btn btn btn-danger rounded" data-toggle="modal" data-target="#modal-delete-{{ $value }}">
            @include('livewire.datatables.icons.trash')
        </button>
    </span>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-delete-label-{{ $value }}">{{ __('datatable.modal_delete_title', ['id' => $value]) }}</h5>
                ...
                <div class="modal-body">
                    <p>
                        {{ __('datatable.modal_delete_info') }}
                    </p>
                    <p class="d-flex justify-content-center">
                        <span x-show="working">@include('livewire.datatables.icons.loading') {{ __('datatable.deleting') }}</span>
                    </p>
                </div>
