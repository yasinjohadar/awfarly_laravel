...
<div class="@if($column['hidden']) hidden @else relative h-12 overflow-hidden align-top flex table-cell @endif">
    ...
    @else
        ...
        <span
            ...
            @if($sort === $index)
                    @if($direction)
                        @include('livewire.datatables.icons.chevron-up', ['classIcon'=>'stroke-current'])
                    @else
                        @include('livewire.datatables.icons.chevron-down', ['classIcon'=>'stroke-current'])
                    @endif
                @endif
        </span>
        </button>
    ...
    <button wire:click.prefetch="toggle('{{ $index }}')"
            class="position-absolute bottom-1 right-1 outline-0">
        @include('livewire.datatables.icons.arrow-circle-left')
    </button>
</div>
