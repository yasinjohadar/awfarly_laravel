<div wire:init="loadScripts">
    <div class="form-group">
        <button class="btn btn-secondary"
                wire:click="$emitUp('setAdvertisementId', null)">{{__('pages/advertisements/slider/show.content.back')}}</button>
    ...
    <div class="form-group">
        <div class="row">
            <div class="col-md-12">
                <div class="row">
                    <div class="col-md-2">{{__('pages/advertisements/slider/show.content.url')}}</div>
                    <div class="col-md-10 font-weight-bold">{{$advertisement->advertisement_url}}</div>
                </div>
                <div class="row">
                    <div class="col-md-2">{{__('pages/advertisements/slider/show.content.starts_at')}}</div>
                    <div class="col-md-10 font-weight-bold">
                        {{$advertisement['starts_at']}}
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-2">{{__('pages/advertisements/slider/show.content.ends_at')}}</div>
                    <div class="col-md-10 font-weight-bold">
                        {{$advertisement['ends_at']}}
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-2">{{__('pages/advertisements/slider/show.content.is_expired')}}</div>
                    <div class="col-md-10 font-weight-bold">
                        {{$advertisement->is_expired ? __("pages/advertisements/slider/show.content.boolean.yes") : __("pages/advertisements/slider/show.content.boolean.no")}}
                    </div>
                </div>
            </div>
            @if($advertisement->getMedia('advertisements')->count() > 0)
                <div class="col-md-12 mt-5">
                    <div class="font-weight-bold">{{__('pages/advertisements/slider/show.content.image')}}</div>
                    <ul id="animated-thumbnails-gallery" dir="ltr"
                        ...
                        @foreach ($advertisement->getMedia('advertisements') as $index => $media)
                            <li data-id="{{$media->id}}" data-src="{{$media->getUrl()}}"
                                ...
                                <div
                                    class="font-weight-bold text-center">{{__('pages/advertisements/slider/show.content.image')}}{{$index+1}}</div>
                            </li>
                ...
                <button style="display: none" id="save_order" class="btn btn-secondary"
                    ...
                    {{__('pages/advertisements/slider/show.content.save')}}
                </button>
            @endif
        </div>
    </div>
    ...
    @endif
    @include('modals.advertisements.slider.edit')
